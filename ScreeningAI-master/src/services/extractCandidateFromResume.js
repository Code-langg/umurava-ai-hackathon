...
exports.extractCandidateFromResumeText = extractCandidateFromResumeText;
...
async function extractCandidateFromResumeText(resumeText, fallbackId) {
    const prompt = buildResumeExtractionPrompt(resumeText);
    const raw = await (0, gemini_1.runGemini)(prompt);
    // Clean response
    const cleaned = raw
        .replace(/```json/g, "")
        .replace(/```/g, "")
